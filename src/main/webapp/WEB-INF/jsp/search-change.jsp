        ...
        <div class="work">
            ...
            <form action="searchChange.action">
	            <div action="#" class="search-work">
	                <input type="text" name="stuId" class="text" placeholder="学号"/>
	            </div>
    ...
    <div class="bloc">
        ...
        <div class="content">
            <table>
                ...
                <c:forEach items="${changeListAll}" var="changeListAll">
						<tr>
						    <td><input type="checkbox" /></td>
							<td>${changeListAll.stuName}</td>
							<td>${changeListAll.stuId}</td>
							<td>${changeListAll.stuClassid}</td>
							<c:if test="${changeListAll.changeStatus==0}">
								<td>调班</td>
							</c:if>
							<c:if test="${changeListAll.changeStatus==1}">
								<td>转学</td>
							</c:if>
							<c:if test="${changeListAll.changeStatus==2}">
								<td>复学</td>
							</c:if>
							<c:if test="${changeListAll.changeStatus==3}">
								<td>休学</td>
							</c:if>
							<c:if test="${changeListAll.changeStatus==4}">
								<td>退学</td>
							</c:if>
							<td><fmt:formatDate value="${changeListAll.date}" pattern="yyyy-MM-dd HH:mm:ss"/></td>
							<td>管理员</td>
							<td class="actions">
								<input name="" type="button" title="Edit this content" class="edittable" />
								<form action="searchChangeDelete.action" method="post" class="deletetable">
									<input type="hidden"  name="searchChangeDelete" value="${changeListAll.stuId}" >
                   ...
                   <c:forEach items="${changeList}" var="item">
						<tr>
							...
							<c:if test="${item.changeStatus==0}">
								<td>调班</td>
							</c:if>
							<c:if test="${item.changeStatus==1}">
								<td>转学</td>
							</c:if>
							<c:if test="${item.changeStatus==2}">
								<td>复学</td>
							</c:if>
							<c:if test="${item.changeStatus==3}">
								<td>休学</td>
							</c:if>
							<c:if test="${item.changeStatus==4}">
								<td>退学</td>
							</c:if>
            ...
            <div id="pageDiv" class="pagination">
					<a href="searchchange.action?pageNo=${page.pageNo-1 }">上一页</a>&nbsp;&nbsp;&nbsp;<a
						href="searchchange.action?pageNo=${page.pageNo+1}">下一页</a>
				</div>
        </div>   
    </div>
    </div>
