...
<body>
            ...
            <ul>
                ...
                <li class="current"><a href="#"><img src="img/icons/menu/books.png" alt="" /> 学生档案</a>
                    <ul>
                        ...
                        <li class="current"><a href="searchfile.action">档案查询</a></li>
                    </ul>
                </li>
                </c:if>
    ...
    <div id="content" class="white">
        ...
        <h1 style="width: 200px;float:left;"><img src="img/icons/posts.png" alt="" /> 档案查询</h1>
        <div class="work">
            <form action="searchFile.action">
        ...
        <div class="content">
            <table>
                ...
                <tbody>
                    ...
                    <c:forEach items="${studentListAll}" var="item">
						<tr>
						    <td><input type="checkbox" /></td>
							<td>${item.stuName}</td>
							<td>${item.stuSex}</td>
							<td>${item.stuId}</td>
							<td>${item.stuAcademy}</td>
							<td>${item.stuDept}</td>
							<td>${item.stuClassid}</td>
							
							<td class="actions">
								<input name="" type="button" title="Edit this content" class="edittable" />
								<form action="searchFileDelete.action" method="post" class="deletetable">
									<input type="hidden"  name="searchFileDelete" value="${item.stuId}" >
        	...
        	<div id="pageDiv" class="pagination">
				<a href="searchfile.action?pageNo=${page.pageNo-1 }">上一页</a>&nbsp;&nbsp;&nbsp;<a href="searchfile.action?pageNo=${page.pageNo+1 }">下一页</a>
			</div>
        </div>   
    </div>
    </div>
